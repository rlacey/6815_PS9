...
def testPainterly(im, texture, outputName, N=100000, size=1, noise=0.3):
    io.imwrite(npr.painterly(im, texture, N, size, noise), str(outputName+"Painterly"+".png"))

def testSingleScaleOrientedPaint(im, texture, outputName, N=10000, size=10, noise=0.3, nAngles=36):
    out = np.zeros_like(im)
    thetas = npr.computeAngles(im)
...
def testOrientedPaint(im, texture, outputName, N=10000, size=1, noise=0.3):
